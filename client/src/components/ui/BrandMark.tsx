export function BrandMark({ compact = false }: { compact?: boolean }) {
  return (
    <div className={`flex items-center ${compact ? 'gap-2' : 'gap-3'}`}>
      <div className={`relative flex ${compact ? 'h-11 w-11 rounded-[18px]' : 'h-14 w-14 rounded-[22px]'} items-center justify-center bg-[radial-gradient(circle_at_top,#9bd06f_0%,#7bb157_32%,#6b4b39_33%,#7c5743_100%)] shadow-soft ring-1 ring-[#f2d8bf]`}>
        ...
        <div className={`absolute inset-x-0 ${compact ? 'top-[14px] gap-1.5' : 'top-[18px] gap-2'} flex justify-center`}>
          <div className={`flex ${compact ? 'h-3.5 w-3.5' : 'h-4 w-4'} items-center justify-center rounded-full bg-[#fff6ea]`}><div className={`${compact ? 'h-2 w-2' : 'h-2.5 w-2.5'} rounded-full bg-[#3c271d]`} /></div>
          <div className={`flex ${compact ? 'h-3.5 w-3.5' : 'h-4 w-4'} items-center justify-center rounded-full bg-[#fff6ea]`}><div className={`${compact ? 'h-2 w-2' : 'h-2.5 w-2.5'} rounded-full bg-[#3c271d]`} /></div>
        </div>
      ...
      {compact && (
        <div className="min-w-0">
          <p className="truncate text-[1rem] font-semibold tracking-tight text-coconut-ink">Coconut Talk</p>
          <p className="truncate text-[10px] text-coconut-shell/68">트로피컬 메신저</p>
        </div>
      )}
    </div>
  );
